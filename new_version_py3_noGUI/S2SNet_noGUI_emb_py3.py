...
def S2SGmain(emb, markov, power, sequences, groups,results,orig_dir):
    ...
    sHeader="Label1\tLabel2\tSequence" #modified !
    if Mark==1:
        for i in range(0,np+1):
            sHeader+="\teSh"+str(i)
    for i in range(0,np+1):
        sHeader+="\teTr"+str(i)
    sHeader+="\teH\teW\teS6\teS"
    if nwi==1:
...
print("*** S2SNet - Sequence to Star Network - command line version ***")
print("@muntisa 2019\n")
print(date_str, "\n")
print("-> Input files in current folder: seqs.txt = sequences; group.txt = characters grouped in branches of SG")
...
print("==>> Results in S2SNetTIs_Emb.txt = descriptors for embedded SG")
print("Done!")
